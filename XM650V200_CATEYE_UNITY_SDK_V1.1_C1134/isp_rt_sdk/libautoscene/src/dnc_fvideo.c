...
static ISP_S32 dnc_to_night(ISP_DEV chn)
{
	...
	stCscAttr.stManual.u8SatuVal = 0;
...
static ISP_S32 dnc_to_day(ISP_DEV chn)
{
	...
	ISP_CSC_ATTR_S stCscAttr;
	ISPCHECK_RET(XM_MPI_ISP_GetCSCAttr(chn, &stCscAttr), "XM_MPI_ISP_GetCSCAttr");
	stCscAttr.stManual.u8SatuVal = 50;
	ISPCHECK_RET(XM_MPI_ISP_SetCSCAttr(chn, &stCscAttr), "XM_MPI_ISP_SetCSCAttr");
...
#ifdef USE_LIBFVIDEO
	ISPCHECK_RET(XM_MPI_CAM_Init(0, (ISP_VOID*)pstConfig),"XM_MPI_CAM_Init");
#else
...
ISP_S32 dnc_init(ISP_DEV IspDev,AUTOSCENE_AE_S *pstAttr)
{
	ISP_U8 u8i;
#ifdef USE_LIBFVIDEO
	//DEBUG("[libautoscene]:use libfvideo chn%d!\n", IspDev);
	CAM_ATTR_PARA stCamAttrPara;
	XM_MPI_ISP_Memset(&stCamAttrPara,0,sizeof(CAM_ATTR_PARA));
	if(NULL != pstAttr)
	{
		for(u8i = 0; u8i < 6; u8i ++)
		{
			stCamAttrPara.astSoftIr[u8i].s32ColorExp = pstAttr->astSoftIr[u8i].s32ColorExp;
			stCamAttrPara.astSoftIr[u8i].s32ColorDlt = pstAttr->astSoftIr[u8i].s32ColorDlt;
			stCamAttrPara.astSoftIr[u8i].s32ColorDlt2 = pstAttr->astSoftIr[u8i].s32ColorDlt2;
			stCamAttrPara.astSoftIr[u8i].s32BwExp = pstAttr->astSoftIr[u8i].s32BwExp;
		}
		stCamAttrPara.u32EshutterLvEn = pstAttr->au32EShutterThr[0];
		stCamAttrPara.u32EshutterLvDis = pstAttr->au32EShutterThr[1];
	}
	ISPCHECK_RET(XM_MPI_CAM_SetPara(IspDev, &stCamAttrPara),"XM_MPI_CAM_SetPara");
#else
	ISP_S32 s32Ret;
	if(!g_monitor_pid)
	{
...
#ifdef USE_LIBFVIDEO
	CAM_DN_STATUS_E enStatus;
	ISPCHECK_RET(XM_MPI_CAM_GetDnStatus(IspDev, &enStatus), "XM_MPI_CAM_GetDnStatus");
	*penStatus = enStatus;
#else
...
#ifdef USE_LIBFVIDEO
	CAM_INNER_INFO stCamInnerInfo;
	ISP_U8 u8i;
	ISPCHECK_RET(XM_MPI_CAM_GetInfo(IspDev, &stCamInnerInfo), "XM_MPI_CAM_GetInfo");
	pstInfo->u8AiStrategyStr = stCamInnerInfo.u8AiStrategyStr;
	pstInfo->u8InfraredStatus = stCamInnerInfo.u8InfraredStatus;
	pstInfo->u8LedStab = stCamInnerInfo.u8LedStab;
	pstInfo->u8DnStatus = stCamInnerInfo.u8DnStatus;
	pstInfo->u32ExpLines = stCamInnerInfo.u32ExpLines;
	pstInfo->u32ExpAllGain = stCamInnerInfo.u32ExpAllGain;
	for(u8i = 0; u8i < 4; u8i ++)
	{
		pstInfo->au8Rsv[u8i] = stCamInnerInfo.au8Rsv[u8i];
		pstInfo->au16Rsv[u8i] = stCamInnerInfo.au16Rsv[u8i];
	}
	pstInfo->au32Rsv[0] = stCamInnerInfo.au32Rsv[0];
	pstInfo->au32Rsv[1] = stCamInnerInfo.au32Rsv[1];
#else
