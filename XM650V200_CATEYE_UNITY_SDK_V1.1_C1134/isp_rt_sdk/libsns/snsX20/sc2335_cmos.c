...
static const ISP_CMOS_AGC_TABLE_S g_stIspAgcTable_sc2335 = {
	/* bvalid */
	1,
...
static ISP_S32 cmos_get_isp_default(ISP_DEV IspDev, ISP_CMOS_DEFAULT_S *pstDef)
{
	...
	gstCmosDnPara[IspDev].pastAgcTab[0] = &g_stIspAgcTable_sc2335;
...
const static ISP_U8 gau8Logic_sc2335[3][1] = {
	//0x363c
	{0x0e},
	{0x07},
	{0x0f},
};
const static ISP_U16 gau16LogicAddr_Mipi[1] = {
	0x363c
};

static ISP_VOID gainLogic_sc2335(ISP_DEV ispDev,ISP_U32 u32AllGain)
{
	static ISP_U8 sau8Idx[ISP_NUM_MAX] = {0xFE};//[bit0~bit3]:Temp   [bit4~bit7]:Agc
	ISP_U8 u8Idx2,u8i;
	if(sau8Idx[0] == 0xFE)
	{
	...
	//gain
	if(0x40 == sensor_read_register(ispDev, 0x3040))
	{
		u8Idx2 = (u32AllGain < 32)? 0 : 1;
	}
	else if(0x41 == sensor_read_register(ispDev, 0x3040))
	{
		u8Idx2 = (u32AllGain < 32)? 2 : 1;
	}
	else
	{
		u8Idx2 = 1;//0x7
	...
	if (u32AllGain >= 96) 
	{	
		sensor_write_register(ispDev, 0x5799, 0x07);
	}
	else if (u32AllGain <= 64) 
	{ 
		sensor_write_register(ispDev, 0x5799, 0x00);
	}
	...
	if( ((sau8Idx[ispDev]>>4)&0x0F) != u8Idx2)
	{
		...
		for(u8i=0; u8i<1; u8i ++)
		{
			sensor_write_register(ispDev, (ISP_U32)gau16LogicAddr_Mipi[u8i], (ISP_U32)gau8Logic_sc2335[u8Idx2][u8i]);
		}
...
static ISP_VOID cmos_gains_update(ISP_DEV IspDev, ISP_U32 u32Again, ISP_U32 u32Dgain)
{
	...
	u32AllGain = (ISP_U64)gau32AGainNow[IspDev]*gau32DGainNow[IspDev]>>10;
	gainLogic_sc2335(IspDev, u32AllGain);
...
static ISP_VOID cmos_fps_set(ISP_DEV IspDev, ISP_U8 u8Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	{
		//just test
		case 6:		u32TotalSizeH = 2640;
					u32TotalSizeV = HD1080_25_LINES*4;
					break;
		//just test
		case 7:		u32TotalSizeH = 2200;
					u32TotalSizeV = HD1080_25_LINES*4;
					break;
		//just test(25fps格式2)
 ...
 * callback structure                                                       *
 ****************************************************************************/
 ISP_S32 cmos_init_chn_sc2335(ISP_DEV IspDev)
{
	return 0;
}
 
ISP_S32 cmos_init_sensor_exp_function_sc2335(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
...
ISP_S32 cmos_init_ae_exp_function_sc2335(AE_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
...
ISP_S32 cmos_init_awb_exp_function_sc2335(AWB_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
