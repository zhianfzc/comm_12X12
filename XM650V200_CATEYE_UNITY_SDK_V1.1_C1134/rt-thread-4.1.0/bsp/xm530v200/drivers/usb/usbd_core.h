...
void usbd_desc_register(struct usb_descriptor *desc, void (*configure_done_callback)(void));
#else
void usbd_desc_register(const uint8_t *desc, void (*configure_done_callback)(void));
...
int usbd_initialize(uint8_t usb_trasns_speed);
int usbd_deinitialize(void);

enum USB_TRANS_SPEED_E
{
	USB_TRANS_SPEED_HS = 0,
	USB_TRANS_SPEED_FS = 1,
};

uint8_t usb_trans_speed_get();
