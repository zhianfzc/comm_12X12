                                            ...
                                            rt_uint32_t              len)
{
    ...
    if (func->card->cccr.multi_block && (len > sdio_max_block_size(func)))
    {
        ...
        while (left_size > func->cur_blk_size)
        {
            ...
            if (ret)
            {
                rt_kprintf("sdio_io_rw_extended error 111\n");
                return ret;
            }
    ...
    while (left_size > 0)
    {
        ...
        if (ret)
        {
            rt_kprintf("sdio_io_rw_extended error 222\n");
            return ret;
        }
...
rt_int32_t sdio_enable_func(struct rt_sdio_function *func)
{
	...
	mmcsd_dbg("SDIO: while  1 read reg = %d\n", reg);
        if (ret)
            goto err;
        if (reg & (1 << func->num))
            break;
                               ...
                               rt_uint32_t              blksize)
{
    ...
    LOG_E("sdio_set_block_size %d",blksize);
    ...
    LOG_E("sdio_set_block_size %d",func->cur_blk_size); 
    return 0;
}
