...
extern void l2x0_inv_range(unsigned long start, unsigned long end);
...
void rt_hw_cpu_dcache_ops(int ops, void *addr, int size)
{
    if (ops == RT_HW_CACHE_FLUSH)
	{
		...
		l2x0_flush_range((unsigned long)addr, (unsigned long)addr + size);
#endif
	}
...
#ifdef RT_USING_L2CACHE
		l2x0_inv_range((unsigned long)addr, (unsigned long)addr + size);
#endif
	}
}
